...
<div class="p-4">
    ...
    <form method="post" action="{{ route('activity.postCreate') }}" enctype="multipart/form-data">
        @csrf
        <div class="mb-6">
            <label for="title"  class="block mb-2 text-md font-medium text-gray-900">Title Activity <span class="text-red" data-tooltip-target="max-title">(Max 200)</span></label>
            ...
            <input type="text" name="title" id="title" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ old('title') }}">
        </div>

        <div class="mb-6">
            <label for="tanggal" class="block mb-2 text-md font-medium text-gray-900">Tanggal</label>
            <input type="date" name="tanggal" id="tanggal" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ old('tanggal') }}">
        </div>

        <div class="flex flex-row gap-6 mb-6">
            <div class="w-full">
                <label for="time_start" class="block mb-2 text-md font-medium text-gray-900">Time Start</label>
                <input type="time" name="time_start" id="time_start" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ old('time_start') }}">
            </div>
            <div class="w-full">
                <label for="time_finish" class="block mb-2 text-md font-medium text-gray-900">Time Finish</label>
                <input type="time" name="time_finish" id="time_finish" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" value="{{ old('time_finish') }}">
            </div>
        </div>
